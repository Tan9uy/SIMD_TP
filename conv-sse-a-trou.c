...
void YCrCb_to_ARGB(uint8_t *YCrCb_MCU[3], uint32_t *RGB_MCU, uint32_t nb_MCU_H, uint32_t nb_MCU_V)
{
   ...
   for (i = 0; i < 8 * nb_MCU_V; i++) {
      for (j = 0; j < 8 * nb_MCU_H; j+=4) {
          ...
          *     _mm_set...
          */
         Y  = _mm_setr_ps(MCU_Y [index], MCU_Y [index + 1], MCU_Y [index + 2], MCU_Y [index+3]);
         Cb = _mm_setr_ps(MCU_Cb[index], MCU_Cb[index + 1], MCU_Cb[index + 2],MCU_Cb[index+3]);
         Cr = _mm_setr_ps(MCU_Cr[index], MCU_Cr[index + 1], MCU_Cr[index + 2], MCU_Cr[index+3] );

         /*
          * Calcul de la conversion YCrCb vers RGB:
          *    aucune instruction sse explicite
          */
         // Rf = _mm_sub_ps(Cr,_mm_set_ps(128,128,128,128));
         // Rf = _mm_mul_ps(Rf,_mm_set_ps(0.381834f,0.381834f,0.381834f,0.381834f));
         // Rf = _mm_add_ps(Rf,Y);

         // Bf = _mm_sub_ps(Cb,_mm_set_ps(128,128,128,128));
         // Bf = _mm_mul_ps(Bf,_mm_set_ps(1.7772f,1.7772f,1.7772f,1.7772f));
         // Bf = _mm_add_ps(Bf,Y);

         // Gf = _mm_sub_ps(Cb,_mm_set_ps(128,128,128,128));
         // Gf = _mm_mul_ps(Gf,_mm_set_ps(0.381834f,0.381834f,0.381834f,0.381834f));
         // Gf = _mm_sub_ps(Gf,_mm_mul_ps(_mm_sub_ps(Cr,_mm_setr_ps(128,128,128,128)), _mm_setr_ps(0.71414f,0.71414f,0.71414f,0.71414f)));
         // Gf = _mm_sub_ps(Y,Gf);
